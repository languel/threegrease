...
export class UI {
  ...
  refreshTimelineControls(): void {
    ...
    controls.replaceChildren(
      ...
      el('div', { class: 'sep' }),
      btn(iconLabel('key', 'Key'), () => this.app.addKeyframe(false), { title: 'Insert blank keyframe (I)' }),
      btn(iconLabel('duplicate', 'Dup'), () => this.app.addKeyframe(true), { title: 'Duplicate current keyframe' }),
      btn(iconLabel('minus', 'Key'), () => this.app.removeKeyframe(), { title: 'Delete keyframe' }),
      el('div', { class: 'sep' }),
      checkbox('Auto-key', ctx.settings.autoKey, (v) => { ctx.settings.autoKey = v; }),
      btn(iconLabel('arrowsRightLeft', 'Interpolate'), () => { interpolateFrame(ctx, ctx.scene.frame, this.interpFactor(ctx)); this.drawTimeline(); }, { title: 'Insert breakdown at current frame' }),
      btn(iconLabel('link', 'Sequence'), () => { interpolateSequence(ctx); this.drawTimeline(); }, { title: 'Interpolate all frames between keys' }),
      el('div', { class: 'sep' }),
      btn(icon('camera'), () => this.app.toggleCameraView(), { active: this.app.cameraView, title: 'Look through the active camera (0)' }),
      this.cameraSelect(),
      btn(iconLabel('plus', 'Cam'), () => this.app.addCamera(), { title: 'Add a camera at the current view' }),
      btn(iconLabel('minus', 'Cam'), () => this.app.removeCamera(), { title: 'Delete the active camera' }),
      checkbox('Lock', this.app.lockCamToView, (v) => { this.app.lockCamToView = v; }),
      btn(iconLabel('key', 'CamKey'), () => this.app.addCameraKey(), { title: 'Keyframe the camera at the current frame' }),
      btn(iconLabel('minus', 'CamKey'), () => this.app.removeCameraKeyAtFrame(), { title: 'Remove camera key at current frame' }),
